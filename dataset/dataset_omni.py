...
class VolumeDataset(Dataset):
    def __init__(
        ...
        clip_input=True,
        bound=0.45,
        mapping_file="./example_data/omni_class.json",
    ):
        super().__init__()
        ...
        with open(mapping_file, "r") as f:
            ...
            self.num_classes = len(self.mapping_dict)
    ...
    def __getitem__(self, idx):
        ...
        if self.uncond_p < 1.0:
            class_label = self.mapping_dict["_".join(os.path.basename(path).split("_")[:-1])]
            class_label = torch.tensor(class_label, dtype=torch.long)
            class_label = torch.tensor(self.num_classes, dtype=torch.long) if (random.random() < self.uncond_p and self.train) else class_label

        relative_transform = None
        data_dict = {"path": path, "class_labels": class_label}
        ...
        if self.load_camera > 0:
            if self.train:
                idxes = np.random.randint(0, 100, size=self.load_camera)
            else:
                idxes = range(0, 100)[:self.load_camera]
            obj_name = os.path.basename(path).split(".")[0]
            class_name = "_".join(os.path.basename(path).split("_")[:-1])
            root_path = os.path.join(self.cam_root_path, class_name, obj_name)
